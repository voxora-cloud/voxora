...
function stripHtmlLikeMarkup(text: string, preserveInteraOne = false) {
  if (!text) return "";
  const rawTagName = preserveInteraOne ? '(?!interaone-)' : '';
  const escapedTagName = preserveInteraOne ? '(?!interaone-)' : '';
  ...
  return text
    ...
    .replace(/<\s*br\s*\/?\s*>/gi, '\n')
    .replace(/&lt;\s*br\s*\/?\s*&gt;/gi, '\n')
    .replace(new RegExp(`<\\/?\\s*${rawTagName}[a-z][a-z0-9:-]*(?:\\s+[^<>]*)?\\/?\\s*>`, 'gi'), '')
    .replace(new RegExp(`&lt;\\/?\\s*${escapedTagName}[a-z][a-z0-9:-]*(?:\\s+[^&]*)?\\/?\\s*&gt;`, 'gi'), '')
    .replace(new RegExp(`<[ \\t]*\\/?[ \\t]*${rawTagName}[a-z][a-z0-9:-]*(?:[ \\t]+[^\\n<>]*)?$`, 'gim'), '')
    .replace(new RegExp(`&lt;[ \\t]*\\/?[ \\t]*${escapedTagName}[a-z][a-z0-9:-]*(?:[ \\t]+[^\\n&]*)?$`, 'gim'), '');
}

function stripHtmlLikeMarkupOutsideCode(text: string, preserveInteraOne = false) {
  const parts = text.split(/(```[\s\S]*?```|`[^`\n]*`)/g);
  return parts
    .map((part) => (
      part.startsWith('```') || part.startsWith('`')
        ? part
        : stripHtmlLikeMarkup(part, preserveInteraOne)
    ))
    .join('');
}
