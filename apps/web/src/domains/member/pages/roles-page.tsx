  ...
  const roles = [
    {
      ...
      description:
        "Full control over the organization. Can manage billing, delete the workspace, and change anyone's role.",
      permissions: [
    ...
    },
    {
      ...
      description:
        "Support staff access. Can interact with assigned conversations and manage their own profile.",
      permissions: [
    ...
    <div className="space-y-8 max-w-5xl">
      <div className="space-y-1">
        ...
        <p className="text-muted-foreground">
          Review the access levels and capabilities for different roles within your
          organization.
        </p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {roles.map((role) => {
          ...
          return (
            <div
              key={role.title}
              className="rounded-xl border border-border bg-card p-6 shadow-sm flex flex-col h-full"
            >
                ...
                <ul className="space-y-2">
                  {role.permissions.map((perm, idx) => (
                    <li key={idx} className="flex items-start">
                      <div
                        className={`mt-1 h-1.5 w-1.5 rounded-full ${role.color} mr-2 shrink-0`}
                      />
