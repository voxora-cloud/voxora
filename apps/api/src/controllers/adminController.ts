...
export const inviteAgent = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!result.success) {
      return sendError(res, result.statusCode || 400, result.message || 'Invite failed');
    }
...
export const updateAgent = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!result.success) {
      return sendError(res, result.statusCode || 400, result.message || 'Update failed');
    }
...
export const deleteAgent = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    if (!result.success) {
      return sendError(res, result.statusCode || 400, result.message || 'Delete failed');
    }
