...
export const initConversation = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    logger.info(`New conversation initialized: ${conversation.id} from ${name} (${email})`);
    ...
    if (socketManagerInstance) {
      ...
      try {
        if (typeof socketManagerInstance.emitToAllUsers === 'function') {
          socketManagerInstance.emitToAllUsers('new_widget_conversation', payload)
        } else if (socketManagerInstance.ioInstance) {
          socketManagerInstance.ioInstance.emit('new_widget_conversation', payload)
        }
        logger.info(`Emitted 'new_widget_conversation' for ${conversation._id}`)
      } catch (emitErr: any) {
        logger.error(`Failed to emit 'new_widget_conversation': ${emitErr?.message || emitErr}`)
      }
    } else {
      logger.warn('Socket manager instance not available; cannot emit new_widget_conversation')
    }
    ...
    sendError(res, 500, 'Failed to initialize conversation: ' + error.message);
  }
});
